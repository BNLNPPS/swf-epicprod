...
Environment:
    SWF_MONITOR_URL   swf-monitor base URL including the /swf-monitor path
                      (as in production.env).
    CORUN_JOBS_URL    corun-ai job-creation REST endpoint.
    CORUN_API_TOKEN   corun-ai API token.

The job-creation payload shape is agreed with corun-ai's owner; adjust
JOB_PAYLOAD if the corun-ai jobs API differs.
"""
...
CORUN_JOBS_URL = os.environ.get('CORUN_JOBS_URL', '').rstrip('/')
...
def main():
    ...
    if not MONITOR_URL:
        ...
        return 2
    if not CORUN_JOBS_URL:
        print('ERROR: CORUN_JOBS_URL is not set', file=sys.stderr)
        log_action('assessment_triggered', outcome='error',
                   reason='CORUN_JOBS_URL unset')
        return 2
    ...
    for campaign in targets:
        payload = {  # JOB_PAYLOAD — shape agreed with corun-ai
            'definition_name': 'epicprod-assessment',
            'params': {
                'campaign': campaign,
                'kind': args.kind,
        ...
        try:
            job = _request(CORUN_JOBS_URL, payload=payload,
