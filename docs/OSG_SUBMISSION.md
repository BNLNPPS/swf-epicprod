...
|---|---|---|---|
| The public `pilot/` prefix of the devcloud bucket, `https://epic-devcloud-stageout.s3.us-east-1.amazonaws.com/pilot/` ([DEVCLOUD_STAGEOUT.md](DEVCLOUD_STAGEOUT.md) § 4): the standing home of our pilots | the Perlmutter pilot launch, `perlmutter/<queue>/epicprod-perlmutter-pilot-launch.sh`, which fetches the tarball at every pilot start and verifies its sha256 ([NERSC_PERLMUTTER.md](NERSC_PERLMUTTER.md)); `BNL_NPPS_GPU`, whose pass script's `PILOTURL` names the object; later any worker outside the SCDF perimeter | one `aws s3 cp` with the worker profile, then a commit on `main` changing the launcher's `PILOT_TARBALL_URL` and `PILOT_TARBALL_SHA256` and the pass script's `PILOTURL`, and the pass script copied to npps0 | Perlmutter: the next worker start after the commit (the launcher is fetched by the site at every worker start); npps0: the next pass after the copy |
| The canary directory, `/cvmfs/eic.opensciencegrid.org/panda/pilot/canary/`, symlink `pilot3.tar.gz` | queues that mount the repository and whose pilot selection names the directory: OSG queues by a `pilot_url` that PanDA operations sets; `BNL_NPPS_GPU` if its `PILOTURL` is pointed there | steps 2 to 6 above (the `cvmfs-canary-publish` procedure) | the next pilot after the repository revision advances, about 10 minutes |
| The npps0 configuration directory, `~/npps0-config/pilot3.tar.gz` on the host | `BNL_NPPS_GPU` when its pass script runs `PILOTURL=local`, for a build not yet published ([NPPS0_TEST_QUEUE.md](NPPS0_TEST_QUEUE.md) § Choosing the pilot) | a file copy to the host (the `npps0` deploy procedure) | the next pass |
