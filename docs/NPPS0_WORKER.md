# npps0 — the GPU worker host for BNL_NPPS_GPU
...
- `tools/npps0/epicprod-gpu-pilot-launcher.sh` — the loop the systemd service
...
- `tools/npps0/epicprod-gpu-pilot.sh` — one pilot pass: environment, workdir
  ...
  action.
- `tools/npps0/epicprod-gpu-pilot.service` — systemd unit running the launcher.
  Installed at `/etc/systemd/system/`.
- `tools/npps0/config/queuedata.json` — the queue's pilot-side behavior,
