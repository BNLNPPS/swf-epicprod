...
The campaign part of the strip is in place, on both tabs: one
completion line per current or producing campaign, from the
completion estimate ([CAMPAIGN_DELIVERY.md](CAMPAIGN_DELIVERY.md),
Completion), served as the cached product
`prod_hub_campaign_completion` (600 s), followed by a labeled link to
the campaign view. The alarm count and credential expiry remain to be
added.
