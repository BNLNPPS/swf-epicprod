...
[PRODUCTION_DEPLOYMENT.md](https://github.com/BNLNPPS/swf-monitor/blob/main/docs/PRODUCTION_DEPLOYMENT.md) (deploying swf-monitor).
...
`run/setup.sh` is bespoke (the SCDF URLs); `etc/panda/` is generated by the
client install.
...
System status is documented in [SYSTEM_STATUS.md](https://github.com/BNLNPPS/swf-monitor/blob/main/docs/SYSTEM_STATUS.md). The page
...
[External Access](https://github.com/BNLNPPS/swf-monitor/blob/main/docs/EXTERNAL_ACCESS.md).
...
under 14 days left; two hours after a task finishes, one of the output
datasets PCS pre-created for it holds fewer files in JLab Rucio than
