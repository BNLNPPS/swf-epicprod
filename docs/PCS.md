...
**Trials.** A trial is a small, real run of a composed configuration, produced by the path production uses — the requesting group's tags, their inputs, real registered data — shown to that group so they accept the configuration before it goes to scale. A configuration goes to scale only after the group has seen its trial and accepted; the acceptance is recorded against a person. A trial is a sample variant of the configuration it proves, taking a compliant unique identity from the machinery that already guards those, and its last segment is the reserved token `TRIAL`: `group.EIC.26.07.1.epic_craterlake.p2339.e1.s1.r1.TRIAL`. Being a variant, it is its own dataset, task and completion unit, so it completes, validates and accounts entirely on its own and touches nothing belonging to the sample it proves. Its scale lives in the variant's submission parameters, and its outputs carry a lifetime: what survives a trial is the acceptance and the record, not the data. The token is the system's alone — a physics variant may never be called `TRIAL` — and a dataset's `trial` flag and its name are two views of one fact, neither permitted to drift from the other. Badges, filters and the accounting exclusions read the flag, never the string.
