...
Because the version segment is part of the identity, each campaign's realization of a sample is its own dataset row — its **edition**. The campaign-invariant identity behind the editions is the **physics configuration**: physics tag + evgen tag (+ background tag) + sample variant — the thing the system is named for, what a requester asks for, and what a disposition decides about ("family" in earlier notes; distinct from a production config, the execution workflow settings). The simu and reco tags are not part of it: they are campaign-supplied software, bound per edition along with the detector config and version segment. A generator version bump deliberately breaks the physics configuration — that is a replacement, recorded with `replaced_by`.

No tag is ever a placeholder that reads as real software. Every intake and import binds the simulation and reconstruction pair whose parameters name the release in the version segment (26.07.1 carries s9.r9, `npsim 26.07.1` and `eicrecon 26.07.1`), minting the pair in the release ladder's form when it is absent (`campaign_stage_tags`); s0 and r0 keep their one meaning, EVGEN-stage request material. The evgen tag is the one the source names — the generator and version derived from the path or the release — and where nothing recorded them the edition carries an unrecorded sentinel that says so in its parameters and its description (`evgen_unrecorded_tag`), never a real tag's version. The 26.07 family was rebound to this rule on 2026-09-08 (`rebind_anchor_editions`; each edition's change in `metadata['rebind']`, one `edition_rebind` action record), which replaced the e1.s1.r1 import and intake anchors on 618 editions; a renamed edition still answers to its former name, from that history. A request row's s0.r0 lasts until the row is about to run: submission and trial composition bind the release pair first (`prodtask_bind_release_tags`, an `edition_bind` record), so the attempt is named and the output registered under the tags of the software that produced it, and nothing submits as s0.r0.

The physics configuration is an entity (`PhysicsConfig`), added when PC-level associations arrived (requesting groups; categorization lenses to follow). Each PC carries a minted short label in the tag vocabulary (`pc147`) — its name in URLs, UI, and service calls — and a canonical identity key computed only by `pcs/physics_config.py`. The identity key uses the semantic evgen identity (generator, version, radiation), not the e-tag label, so editions of one PC merge across campaigns even when their e-tag numbering differs. Rows are created and bound automatically as editions save — composition implies the configuration; nothing else creates one — and every edition carries a `physics_config` reference, so an edition is a physics configuration × campaign. The PC page (`/pcs/config/<label>/`) presents the identity read-only and is the curation surface for associations, which are set only through the service layer with a required comment and append-only history. The intended end state is an edition slimmed to the PC reference plus campaign-supplied bindings; identity fields on the edition remain until every consumer reads them through the reference.

Two dataset fields plan the configuration's future across campaigns:
...
**Scale and outputs.** A trial runs one job of its manifest at its own event count, 100 by default, against the real input. Its FULL, RECO and LOG outputs land under `epic:/TEST/trial/<trial name>` in the production layout — the same substructure beneath a root that cannot be mistaken for production, and one root per trial, since the payload names an output by its directory and input file and a shared root would give every trial of a configuration the same DID — and everything it registers carries a lifetime, two weeks by default. What survives a trial is the acceptance and the record, not the data.
