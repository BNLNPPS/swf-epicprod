...
1. **Choose an external-safe trigger.** Most collaborators reach the agent
   ...
   [EXTERNAL_ACCESS.md](https://github.com/BNLNPPS/swf-monitor/blob/main/docs/EXTERNAL_ACCESS.md) → *Write actions and triggers*, and
...
| `cric_declared_state` | `cric-declared-state.py` | The declared record from CRIC (CONTINUOUS_PRODUCTION.md, Declared downtime): the PanDA queue status rules, the DDM endpoint status rules and the downtime windows of the EIC queues, endpoints and sites, read with the agent's proxy and kept in the entry store with their standing (active, future, expired, cleared); the change is a `declared_state_sync` action. Ten-minutely by cron enqueue (`5-55/10 * * * *`, log `declared-state-cron.log`). | 300 |
...
- **Proactive completion notification** — auto-notify the operator the moment a
  ...
  callback is the working model. Browser-push design: [SSE_PUSH.md](https://github.com/BNLNPPS/swf-monitor/blob/main/docs/SSE_PUSH.md)
